	...
	App.view = {
		...
		initScroller: function(){
			...
			if (typeof(IScroll) == 'function') {
				...
				//events
				App.view.Scroller.on('beforeScrollStart', 	function(){ });
				App.view.Scroller.on('scrollStart', 		function(){ }); 
				App.view.Scroller.on('scrollEnd', 			function(){ });
		...
		initArticleScroller: function( $article ){
			...
			if (Platform.iOS && $container.hasClass('native')) { return 'use native scroller instead of plugin'; }
			//destroy scroller instance
			if (App.view.articleScroller) App.view.articleScroller.destroy();
			//init scrolling plugin
			if (typeof(IScroll) == 'function') {
				App.view.articleScroller = new IScroll($container[0], {
					scrollbars: 			true,
					useTransition:			Platform.iOS ? false : true, //true,
					useTransform:			Platform.iOS ? false : true, //true,
					HWCompositing:			Platform.iOS ? false : true //true
				});
				//events
				App.view.articleScroller.on('beforeScrollStart', 	function(){ });
				App.view.articleScroller.on('scrollStart', 			function(){ }); 
				App.view.articleScroller.on('scrollEnd', 			function(){ });
			}
		},
		...
		initWeeksNavTabs: function(){
			...
			$btnTabs.on('tap', function(e){
				...
				//exit and jump to landing, if click on current btn
				if ( $(this).hasClass(currentCLS) ) { 
					App.view.onJumpStraightToLanding();
					return 'target week already current, jumping back to landing';
				}
				//vars
				var $this = $(this),
		...
		//onJumpStraightToLanding
		onJumpStraightToLanding: function(){
			//vars
			var targetIndex = 0,
				typeIsLanding = true,
				seg = App.view.weeksScroller.currentSegment.x,
				$article = $('#weekContainer' + (seg+1)).find('[data-role="article"]'),
				currentIndex = parseInt($article.attr('data-index'), 10);
			
			//exit if already on landing
			if (targetIndex === currentIndex) {
				return 'already on landing';
			}
			...
			//load article
			console.log('[onJumpStraightToLanding]: index:', targetIndex);
			App.view.loadWeekArticle(seg, targetIndex);
		},
			...
			function onBtnAudio(e){
				...
				if (networkState == 'none' || networkState == 'unknown') {
					...
					navigator.notification.alert(
						// message
						'The Lectio Divina App requires internet connection to playback audio. Please make sure your device has a working connection and try again.',
						// callback
						function(){},
						// title
